    ...
    if (type === 'user') {
      ...
      if (toUser) {
        ...
        if (toUser.robot) {
          ...
          socket.emit('message', {
            from: to, to: from, content: data.results[0].values.text, type
          })
          console.log(data)
...
//   console.log('API Server is running. http://127.0.0.1:5500')
// })

httpServer.listen(process.env.PORT || 5500, '0.0.0.0', () => {
  console.log('API Server is running. http://127.0.0.1:5500')
})
